...
| **Dispatch Context** | The per-event runtime bundle containing the cloned `WappaEventHandler` and available capabilities. Messaging events require an Inbox Reference; an Inbox-independent external event carries DB access without Messenger, Inbox cache, User, or SSE identity. |
...
| **Authentication Template** | A Template in the `authentication` category. Meta fixes its body to a single positional code placeholder, forbids a media or location header, will not deliver it to a BSUID address, and requires an OTP Button. Wappa refuses a request that breaks any of those locally rather than paying a provider round trip. |
| **OTP Button** | The button component an Authentication Template send must carry beside its body. Meta rewrites the `OTP` button to a `URL` button at template creation, so every Authentication Method sends `sub_type: "url"` with a string `index` and one text parameter repeating the code the body already carries. |
| **Authentication Method** | How the recipient's device consumes an OTP Button's code: `copy_code` to the clipboard, `one_tap` handed to a registered app on tap, `zero_tap` broadcast without a tap. It is a property of the approved template, not of the send; all three produce the same outgoing payload. The device handshake and `supported_apps` metadata belong to template creation and the Host Application's mobile app. |
...
| **Webhook ID** | An optional Host-chosen routing value in an External Webhook Source URL. It is opaque to Wappa and never identifies, selects, or authorizes an Inbox. |
| **External Event** | The authenticated, source-specific fact produced from an External Webhook Source request. It carries source data and an optional Webhook ID, never Inbox or User identity. |
| **External Webhook Context Resolver** | A Host-provided strategy that may associate an authenticated External Event with one concrete Inbox Reference and optional User using payload, headers, database state, or trusted FastAPI request state. |
| **External Webhook Admission** | The synchronous boundary that authenticates and parses an External Webhook Source request, resolves optional context, and binds a handler before Wappa acknowledges and schedules dispatch. |
| **External Webhook Runtime** | The Wappa module that admits an External Webhook Source request and dispatches its External Event with either resolved Inbox capabilities or database-only context. |
