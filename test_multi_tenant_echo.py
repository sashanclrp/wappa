...
class MultiTenantEchoHandler(WappaEventHandler):
    ...
    def __init__(self):
        ...
        self.logger.info("🚀 MultiTenantEchoHandler initialized - dependencies will be injected per request")
    ...
    async def process_message(self, webhook: IncomingMessageWebhook) -> None:
        ...
        if not self.validate_dependencies():
            self.logger.error("❌ Dependencies not properly injected - cannot process message")
            return
        ...
        self.logger.info(f"💬 Request #{self._request_count}: Processing {message_type} from {user_id}")
        ...
        if raw_data:
            self.logger.debug(f"📄 Raw webhook data available (keys: {list(raw_data.keys())})")
    ...
    async def _handle_text_echo(self, webhook: IncomingMessageWebhook, user_id: str, message_text: str) -> None:
    ...
    async def _handle_media_echo(self, webhook: IncomingMessageWebhook, user_id: str, message_type: str) -> None:
        ...
        if result.success:
            self.logger.info(f"✅ {message_type.title()} metadata sent: {result.message_id}")
        else:
            self.logger.error(f"❌ {message_type.title()} metadata failed: {result.error}")

    async def _handle_location_echo(self, webhook: IncomingMessageWebhook, user_id: str) -> None:
        """Handle location message echo."""
        self.logger.info(f"📍 Processing location echo")
    ...
    async def _handle_contact_echo(self, webhook: IncomingMessageWebhook, user_id: str) -> None:
        """Handle contact message echo."""
        self.logger.info(f"👥 Processing contact echo")
    ...
    async def _handle_unsupported_echo(self, webhook: IncomingMessageWebhook, user_id: str, message_type: str) -> None:
    ...
    def _build_message_metadata_response(self, webhook: IncomingMessageWebhook, message_type: str) -> str:
