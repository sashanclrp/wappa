...
class DatabasePlugin:
    ...
    async def startup(self, app: "FastAPI") -> None:
        ...
        try:
            # Create async engine
            logger.debug(f"Creating database engine with adapter: {self.adapter.__class__.__name__}")
            ...
            self.session_factory = await self.adapter.create_session_factory(self.engine)
            ...
            if self.initialize_schema and self.models:
                ...
                logger.info(f"Database schema initialized for models: {[m.__name__ for m in self.models]}")
