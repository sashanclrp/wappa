...
| **SSEEventContext** | The per-request `ContextVar` value carrying `inbox_id` (field currently named `tenant_id`), `user_id`, `bsuid`, `phone_number`, `platform`, and a free-form `metadata` bag. Set once by the framework entry point; enriched by pipeline middleware. |
...
| **SSELifecycleMiddleware** | Messenger Pipeline middleware (priority 70) that flushes any Pending Incoming event, awaits the outgoing send, then publishes `outgoing_bot_message` to the hub. Replaces the deprecated `SSEMessengerWrapper`. |
...
| **PubSubNotificationMiddleware** | Messenger Pipeline middleware (priority 30) that publishes a `bot_reply` PubSub Notification after every successful outgoing send. Reads identity from the active `SSEEventContext`. Replaces the deprecated `PubSubMessengerWrapper`. |
