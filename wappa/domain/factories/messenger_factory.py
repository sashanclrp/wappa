...
class MessengerFactory:
    ...
    async def create_messenger(
        ...
        self.logger.debug(f"Creating new messenger for platform: {platform.value}, tenant: {tenant_id}")
        ...
        try:
            ...
            self.logger.error(f"Failed to create messenger for platform {platform.value}: {e}")
    ...
    async def _create_whatsapp_messenger(self, tenant_id: str) -> WhatsAppMessenger:
        ...
        interactive_handler = WhatsAppInteractiveHandler(client=client, tenant_id=tenant_id)
        template_handler = WhatsAppTemplateHandler(client=client, tenant_id=tenant_id)
        specialized_handler = WhatsAppSpecializedHandler(client=client, tenant_id=tenant_id)
        ...
        self.logger.info(f"✅ WhatsApp messenger created successfully for tenant: {tenant_id}")
        return messenger
    ...
    def clear_cache(self, platform: PlatformType = None, tenant_id: str = None):
        ...
        if platform and tenant_id:
            ...
            to_remove = [key for key in self._messenger_cache.keys() if key.startswith(f"{platform.value}:")]
            ...
            to_remove = [key for key in self._messenger_cache.keys() if key.endswith(f":{tenant_id}")]
            for key in to_remove:
                del self._messenger_cache[key]
